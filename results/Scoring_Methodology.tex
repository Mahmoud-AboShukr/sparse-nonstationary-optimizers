...
\title{Scoring Methodology for Optimizer Comparison}
\author{}
\date{}
...
\begin{equation}
z_i = \frac{x_i - \text{median}(X)}{IQR(X)}
\end{equation}
...
\begin{equation}
\text{Score}_i = 50 + 45 \cdot \frac{\tanh(z_i/2)}{\tanh(1)}
\end{equation}

This approach offers three key advantages: 
\begin{enumerate}
    \item robustness to outliers through use of the IQR rather than range; 
    \item graceful handling of tight metric clustering by avoiding artificial binary distinctions; 
    \item interpretability through bounded scores in a fixed $[1, 99]$ range.
\end{enumerate}

\subsection{Interpretation of Extreme Scores}
...
\begin{itemize}
    \item \textbf{lamb's Stability Score of 99.0} corresponds to an Update Stability value of $0.294$, which is the minimum across all tested optimizers, indicating genuinely superior parameter stability.
    
    \item \textbf{stable-spam's Stability Score of 1.0} reflects an Update Stability value of $0.315$, representing the maximum and thus poorest stability performance within the main optimizer cluster.
    
    \item \textbf{came's Convergence Score of 1.0} corresponds to a Loss Stability value of $1.73$, substantially exceeding the median ($0.89$) and indicating genuine instability in loss trajectory convergence.
\end{itemize}
